...
StandardOperatorTaskGroup pool2d_get_task_group(
    ...
    ParallelTensorDimDegrees const &input_degrees)
{
  parallel_tensor_dim_idx_t batch_dim = shard_dim_idx(ff_dim_t{0_n});

  for (parallel_tensor_dim_idx_t const &dim_idx : get_parallel_tensor_dim_indices(input_degrees)) {
    if (dim_idx != discard_copy_dim_idx() && dim_idx != batch_dim) {
      positive_int degree = get_degree_for_parallel_tensor_dim_idx(input_degrees, dim_idx);
      ASSERT(degree == 1_p,
             "Currently pool2d_get_task_group only handles a limited set of types of parallelism. "
             "If you need support for additional types, please create an issue."
      );
    }
  }

  ParallelTensorDimDegrees output_degrees =
    pool2d_get_output_parallel_dim_degrees(attrs, input_degrees);
          ...
          ParallelTensorDimDegrees const &input_degrees)
{
  StandardOperatorTaskGroup op_task_group =
    pool2d_get_task_group(attrs, input_degrees);
