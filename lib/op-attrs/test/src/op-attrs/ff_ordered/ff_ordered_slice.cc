...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("ff_ordered_slice(FFOrdered<T>, ..., ...") {
    ...
    SUBCASE("ff_dim_t, ff_dim_t") {
      FFOrdered<size_t> result = ff_ordered_slice(
          d, ff_dim_t{nonnegative_int{1}}, ff_dim_t{nonnegative_int{3}});
    ...
    SUBCASE("ff_dim_t, std::nullopt_t") {
      FFOrdered<size_t> result =
          ff_ordered_slice(d, ff_dim_t{nonnegative_int{1}}, std::nullopt);
    ...
    SUBCASE("relative_ff_dim_t, relative_ff_dim_t") {
      FFOrdered<size_t> result =
          ff_ordered_slice(d, relative_ff_dim_t{1}, relative_ff_dim_t{-1});
      FFOrdered<size_t> correct = FFOrdered<size_t>{2, 3};
    ...
    SUBCASE("relative_ff_dim_t, std::nullopt_t") {
      FFOrdered<size_t> result =
          ff_ordered_slice(d, relative_ff_dim_t{-3}, std::nullopt);
      FFOrdered<size_t> correct = FFOrdered<size_t>{2, 3, 4};
    ...
    SUBCASE("start index = stop index (using negative indexing)") {
      FFOrdered<size_t> result =
          ff_ordered_slice(d, relative_ff_dim_t{1}, relative_ff_dim_t{-3});
      FFOrdered<size_t> correct = FFOrdered<size_t>{};
    ...
    SUBCASE("start index > stop index") {
      FFOrdered<size_t> result =
          ff_ordered_slice(d, relative_ff_dim_t{1}, relative_ff_dim_t{0});
      FFOrdered<size_t> correct = FFOrdered<size_t>{};
    ...
    SUBCASE("start index > stop index (using negative indexing)") {
      FFOrdered<size_t> result =
          ff_ordered_slice(d, relative_ff_dim_t{1}, relative_ff_dim_t{-4});
      FFOrdered<size_t> correct = FFOrdered<size_t>{};

      CHECK(result == correct);
    }
    SUBCASE("start index out of bounds (too low)") {
      CHECK_THROWS(ff_ordered_slice(d, relative_ff_dim_t{-10}, std::nullopt));
    }
    ...
    SUBCASE("stop index out of bounds (too low)") {
      CHECK_THROWS(
          ff_ordered_slice(d, relative_ff_dim_t{0}, relative_ff_dim_t{-10}));
    }
