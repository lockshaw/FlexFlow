...
std::unordered_map<OperatorAttributeKey, OperatorAttributeValue> get_attribute_map(PCGOperatorAttrs const &op_attrs) {
  ...
  for (OperatorAttributeKey const &attr_key : all_operator_attribute_keys()) {
    std::optional<OperatorAttributeValue> attr_value = get_attribute(op_attrs,  attr_key);
