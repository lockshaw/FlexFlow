...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("SubstitutionBuilder") {
    ...
    std::unordered_map<OperatorAttributeKey, OutputOperatorAttributeExpr> fused_mm_relu_attr_assignments = {
    ...
    Substitution result = [&] {
      ...
      OutputGraphExprValue o_fused_output = get_only(
        b.add_output_graph_node(fused_mm_relu_attrs_assignment, {o_input, o_weight}, 1)
      );
