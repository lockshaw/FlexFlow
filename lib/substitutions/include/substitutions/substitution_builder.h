...
public:
  ...
  std::pair<PatternValue, OutputGraphExprValue> add_input(TensorAttributePattern const &, std::optional<std::string> const &name = std::nullopt);
  void equate_outputs(PatternValue const &, OutputGraphExprValue const &);

  std::vector<PatternValue> add_pattern_node(OperatorAttributePattern const &node_pattern, 
                                             std::vector<PatternValue> const &inputs,
                                             std::vector<TensorAttributePattern> const &output_patterns,
                                             std::optional<std::string> const &name = std::nullopt);

  std::vector<OutputGraphExprValue> add_output_graph_node(OutputOperatorAttrsAssignment const &node_expr,
