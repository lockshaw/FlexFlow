...
SearchResult apply_data_parallelism(ComputationGraph const &cg,
  ...
  auto parallelize_tensor = [&](parallel_tensor_guid_t t, ParallelTensorDimDegrees degrees) -> parallel_tensor_guid_t {
    ASSERT(get_total_parallel_degree(get_parallel_tensor_shape(pcg, t)) == 1);

    for (ParallelOpAttrs const &parallel_op : generate_weight_transform(degrees)) {
      t = require_only_key(pcg_add_parallel_op_layer(pcg, parallel_op, t).outputs, TensorSlotName::OUTPUT);
    }
  ...
  auto add_layer_to_pcg = [&](layer_guid_t const &layer) -> void {
    ...
    std::unordered_map<TensorSlotName, ParallelTensorDimDegrees>
      input_degrees = map_values(
        inputs,
        [&](parallel_tensor_guid_t t) -> ParallelTensorDimDegrees {
          return get_parallel_degrees(get_parallel_tensor_shape(pcg, t));
        });

    std::unordered_map<TensorSlotName, ParallelTensorDimDegrees>
      weight_degrees = infer_weight_degrees(parallel_layer_attrs.op_attrs, input_degrees);

    std::unordered_map<TensorSlotName, parallel_tensor_guid_t> weights = map_values(
    ...
    std::unordered_map<TensorSlotName, parallel_tensor_guid_t>
      parallelized_weights = zip_values_strict_with(
        weights,
        weight_degrees,
        parallelize_tensor);
