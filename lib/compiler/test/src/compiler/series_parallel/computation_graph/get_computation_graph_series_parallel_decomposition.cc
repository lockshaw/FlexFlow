...
TEST_CASE(
  ...
  SECTION("empty computation graph") {
  ...
  SECTION("single operator plus inputs and weights") {
    std::string input_layer_name = "my input";
  ...
  SECTION("SP without weight nodes but non-SP with weight nodes") {
  ...
  SECTION("SP with or without preprocessing, but preprocessing would SP "
          "decomposition") {
    // computation graph:
    //
    //  input1   input2
  ...
  SECTION("not SP with or without weight nodes") {
    // computation graph:
    //
    //    input1
    //     /  \
...
TEST_CASE("render_preprocessed_computation_graph_for_sp_decomposition("
  ...
  SECTION("basic single-operator model") {
    ComputationGraph cg = [&] {
      ComputationGraphBuilder b;
  ...
  SECTION("split_test") {
    ...
    BENCHMARK("split_test") {
      return get_computation_graph_series_parallel_decomposition(cg);
    };
  }
