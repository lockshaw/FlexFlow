...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("dynamic_op_dataflow_graph_from_invocation_set") {
    ...
    auto mk_dynamic_value = [](size_t node_id, TensorSlotName slot_name)
      -> DynamicValueAttrs
    {
    ...
    SUBCASE("throws if multiple invocations produce the same value") {
      ...
      CHECK_THROWS(dynamic_open_dataflow_graph_from_invocation_set(invocation_set));
    }
  ...
  TEST_CASE("get_dynamic_slot_sites") {
    auto mk_dynamic_value = [](int node_id, TensorSlotName slot_name)
      -> DynamicValueAttrs
    {
      return DynamicValueAttrs{
          /*tensor_guid=*/dynamic_tensor_guid_t{parallel_tensor_guid_t{
    ...
    std::unordered_set<DynamicSlotSite> correct = {
      ...
      mk_internal_slot_site(invocation_1, TensorDirection::INCOMING, TensorSlotName::INPUT),
      mk_internal_slot_site(invocation_1, TensorDirection::OUTPUT, TensorSlotName::OUTPUT),
      mk_internal_slot_site(invocation_2, TensorDirection::INCOMING, TensorSlotName::INPUT),
      mk_internal_slot_site(invocation_2, TensorDirection::INCOMING, TensorSlotName::WEIGHT),
    };

    CHECK(result == correct);
  }

  TEST_CASE("labelled_open_kwarg_dataflow_graph_from_dynamic_open_dataflow_graph") {
    ...
    DynamicTensorSlot fwd_weight_output_slot1 = DynamicTensorSlot{
      /*pcg_slot_name=*/TensorSlotName::OUTPUT,
    ...
    DynamicTensorSlot upd_weight_input_slot3 = DynamicTensorSlot{
      /*pcg_slot_name=*/TensorSlotName::OUTPUT,
      /*slot_tensor_role=*/mk_dynamic_tensor_role_opt(OptimizerSlotName::SGD_V),
    };
    ...
    std::pair<LabelledOpenKwargDataflowGraph<DynamicNodeAttrs,
      ...
      result = labelled_open_kwarg_dataflow_graph_from_dynamic_open_dataflow_graph(input);
