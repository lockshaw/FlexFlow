...
bidict<MultiDiEdge, std::pair<OutputMultiDiEdge, InputMultiDiEdge>>
    get_edge_splits(T const &t, GraphSplit const &s) {
  return get_edge_splits(t, create_tag(t), s);
}

template <typename T>
bidict<MultiDiEdge, std::pair<OutputMultiDiEdge, InputMultiDiEdge>>
    get_edge_splits(T const &t, graph_subtype<MultiDiGraphView> const &tag, GraphSplit const &s) {
  return impl_get_edge_splits(coerce(t, tag), s);
}

bidict<MultiDiEdge, std::pair<OutputMultiDiEdge, InputMultiDiEdge>>
    impl_get_edge_splits(MultiDiGraphView const &, GraphSplit const &);
