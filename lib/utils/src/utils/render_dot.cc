...
std::string render_dot_node_attrs(std::unordered_map<std::string, std::string> const &node_attrs) {
  std::ostringstream oss;
  for (auto const &[k, v] : node_attrs) {
    oss << fmt::format("\"{}\"=\"{}\",", escape_dot_string(k), escape_dot_string(v));
  }
  return oss.str();
}

std::string render_node_label(LabelledDataflowGraphView<std::unordered_map<std::string, std::string>, std::string> const &g, Node const &n) {
  ...
  RecordFormatter rec;
  rec << inputs_record 
      << try_at(g.at(n), std::string{"label"}).value_or(fmt::to_string(n.raw_uid)) 
      << outputs_record;
...
std::string render_dot(LabelledDataflowGraphView<std::unordered_map<std::string, std::string>, std::string> const &g) {
  ...
  for (Node const &n : get_nodes(g)) {
    ...
    lines.push_back(fmt::format("  n{} [{}];", n.raw_uid, render_dot_node_attrs(node_attrs)));
  }

  for (DataflowEdge const &e : get_edges(g)) {
    lines.push_back(fmt::format("  n{}:o{} -> n{}:i{};", e.src.node.raw_uid, e.src.idx, e.dst.node.raw_uid, e.dst.idx));
  }
