...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("view_as_closed_kwarg_dataflow_graph_by_materializing_inputs") {
    OpenKwargDataflowGraphView<std::string, int> open_g = [] {
      OpenKwargDataflowGraph<std::string, int>
        g = OpenKwargDataflowGraph<std::string, int>::template create<
      ...
      KwargNodeAddedResult<int> n1_added = g.add_node(
        /*inputs=*/std::unordered_map<int, OpenKwargDataflowValue<std::string, int>>{
          {
            1,
      ...
      KwargDataflowOutput<int> n1_output = require_only_key(n1_added.outputs, 5);
      ...
      KwargDataflowOutput<int> n2_output = require_only_key(n2_added.outputs, 5);

      return g;
    }();

    std::pair<
      KwargDataflowGraphView<std::optional<int>>,
      bidict<KwargDataflowGraphInput<std::string>, Node>
    >
      result = view_as_closed_kwarg_dataflow_graph_by_materializing_inputs(open_g);
    ...
    KwargDataflowGraphView<std::optional<int>> correct = [] {
      ...
      KwargDataflowOutput<std::optional<int>> input1 = require_only_key(input1_added.outputs, std::optional<int>{std::nullopt});
      ...
      KwargDataflowOutput<std::optional<int>> input2 = require_only_key(input2_added.outputs, std::optional<int>{std::nullopt});
      ...
      KwargDataflowOutput<std::optional<int>> n1_output = require_only_key(n1_added.outputs, std::optional<int>{5});
    ...
    ASSERT(get_all_kwarg_dataflow_edges(result_g).size() == get_all_open_kwarg_dataflow_edges(open_g).size());
