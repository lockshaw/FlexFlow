...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("view_from_kwarg_dataflow_graph_data") {
    auto mk_edge = [](Node src, std::optional<int> src_idx, Node dst, std::optional<int> dst_idx)
      -> KwargDataflowEdge<std::optional<int>>
    {
      return KwargDataflowEdge<std::optional<int>>{
        ...
        /*dst=*/KwargDataflowInput<std::optional<int>>{
          /*node=*/dst,
          /*slot_name=*/dst_idx,
        },
      };
    ...
    KwargDataflowGraphData<std::optional<int>> data = KwargDataflowGraphData<std::optional<int>>{
      /*nodes=*/{ n0, n1, n2 },
      /*edges=*/all_edges,
      /*outputs=*/all_outputs,
    };

    KwargDataflowGraphView<std::optional<int>> g = view_from_kwarg_dataflow_graph_data(data);
    ...
    SUBCASE("get_all_kwarg_dataflow_edges") {
      std::unordered_set<KwargDataflowEdge<std::optional<int>>> result = get_all_kwarg_dataflow_edges(g);
      std::unordered_set<KwargDataflowEdge<std::optional<int>>> correct = all_edges;
      ASSERT(result == correct);
    }

    SUBCASE("get_all_kwarg_dataflow_outputs") {
      std::unordered_set<KwargDataflowOutput<std::optional<int>>> result = get_all_kwarg_dataflow_outputs(g);
      std::unordered_set<KwargDataflowOutput<std::optional<int>>> correct = all_outputs;
      ASSERT(result == correct);
    }
  }
}
