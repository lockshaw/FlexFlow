  ...
  TEST_CASE("dataflow_graph_from_kwarg_dataflow_graph") {
    ...
    KwargDataflowGraphView<std::string> input = [] {
      ...
      KwargNodeAddedResult<std::string> n0_added = g.add_node(
        /*inputs=*/std::unordered_map<std::string, KwargDataflowOutput<std::string>>{},
        /*outputs=*/std::unordered_set<std::string>{
          "a",
        });

      KwargDataflowOutput o0 = require_only_key(n0_added.outputs, std::string{"a"});
    ...
    std::function<std::vector<std::string>(std::unordered_set<std::string> const &)> slot_ordering 
      = [](std::unordered_set<std::string> const &slots) -> std::vector<std::string> {
        return reversed(sorted(slots));
      };

    DataflowGraphView result = dataflow_graph_from_kwarg_dataflow_graph(input, slot_ordering);
