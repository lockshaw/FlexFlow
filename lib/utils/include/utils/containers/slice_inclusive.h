#ifndef _FLEXFLOW_LIB_UTILS_INCLUDE_UTILS_CONTAINERS_SLICE_INCLUSIVE_H
#define _FLEXFLOW_LIB_UTILS_INCLUDE_UTILS_CONTAINERS_SLICE_INCLUSIVE_H
...
template <typename T>
std::vector<T> slice_inclusive(std::vector<T> const &v,
                               int maybe_start,
  ...
  if (maybe_end.has_value()) {
    end_iter = v.cbegin() + resolve_loc(maybe_end.value()) + 1;
  }
