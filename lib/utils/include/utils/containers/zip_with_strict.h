...
std::vector<Result> zip_with_strict(std::vector<T1> const &lhs, std::vector<T2> const &rhs, F &&f) {
  if (lhs.size() != rhs.size()) {
    throw mk_runtime_error(fmt::format("zip_with_strict requires inputs to have the same length, but received lhs = {} (length {}) and rhs = {} (length {})", lhs, lhs.size(), rhs, rhs.size()));
  }
