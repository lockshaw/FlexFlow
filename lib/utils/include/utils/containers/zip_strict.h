...
std::vector<std::pair<L, R>> zip_strict(std::vector<L> const &lhs,
  ...
  if (lhs.size() != rhs.size()) {
    throw mk_runtime_error(fmt::format("zip_strict requires lhs and rhs to have the same length, but received lhs={} (length {}), rhs={} (length {})", lhs, lhs.size(), rhs, rhs.size()));
  }
