...
struct UnorderedSetKwargDataflowGraph final
  ...
  void add_node_unsafe(
      ...
      std::unordered_map<SlotName, KwargDataflowOutput<SlotName>> const &outputs) override
  {
  ...
  void inplace_materialize_from(KwargDataflowGraphView<SlotName> const &v) override {
...
private:
  UnorderedSetKwargDataflowGraph(
      ...
      std::unordered_set<KwargDataflowEdge<SlotName>> const
          &edges,
      std::unordered_set<KwargDataflowOutput<SlotName>> const &outputs)
      : node_source(node_source), nodes(nodes),
