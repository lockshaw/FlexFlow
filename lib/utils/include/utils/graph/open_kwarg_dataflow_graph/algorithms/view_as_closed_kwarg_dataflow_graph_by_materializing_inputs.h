    ...
    OpenKwargDataflowGraphView<GraphInputName, SlotName> const &g)
{
  ...
  bidict<
    ...
    Node
  > graph_input_nodes = generate_bidict(open_g_data.inputs,
                                        [&](KwargDataflowGraphInput<GraphInputName> const &) -> Node {
                                          return n.new_node();
                                        });

  auto kwarg_dataflow_output_for_graph_input = [&](KwargDataflowGraphInput<GraphInputName> const &i)
  ...
  auto convert_kwarg_dataflow_output = [&](KwargDataflowOutput<SlotName> const &o)
    -> KwargDataflowOutput<std::optional<SlotName>>
  {
    return KwargDataflowOutput<std::optional<SlotName>>{
      /*node=*/o.node,
  ...
  auto convert_edge = [&](OpenKwargDataflowEdge<GraphInputName, SlotName> const &open_edge)
    -> KwargDataflowEdge<std::optional<SlotName>>
  {
    return open_edge.template visit<KwargDataflowEdge<std::optional<SlotName>>>(overload{
  ...
  KwargDataflowGraphData<std::optional<SlotName>> closed_g_data
    = KwargDataflowGraphData<std::optional<SlotName>>{
      /*nodes=*/set_union(open_g_data.nodes, right_entries(graph_input_nodes)),
      /*edges=*/transform(open_g_data.edges, convert_edge),
      /*outputs=*/set_union(
        transform(open_g_data.outputs, convert_kwarg_dataflow_output),
        transform(open_g_data.inputs, kwarg_dataflow_output_for_graph_input)),
    };
