...
std::string
  open_kwarg_dataflow_graph_as_dot(OpenKwargDataflowGraphView<GraphInputName, SlotName> const &g)
{
  ...
  std::function<nlohmann::json(OpenKwargDataflowValue<GraphInputName, SlotName> const &)>
    render_value = [](OpenKwargDataflowValue<GraphInputName, SlotName> const &v) {
      nlohmann::json j = fmt::to_string(v);
      return j;
    };

  std::function<nlohmann::json(SlotName const &)> render_slot_name = [](SlotName const &s) {
    nlohmann::json j = fmt::to_string(s);
    return j;
  };

  std::function<std::vector<SlotName>(std::unordered_set<SlotName> const &)> order_slots =
    [](std::unordered_set<SlotName> const &unordered) {
...
std::string
  open_kwarg_dataflow_graph_as_dot(
    ...
    std::function<nlohmann::json(OpenKwargDataflowValue<GraphInputName, SlotName> const &)> const &render_value,
    ...
    std::function<std::vector<SlotName>(std::unordered_set<SlotName> const &)> const &order_slots)
{
  ...
  KwargDataflowGraphView<std::optional<SlotName>> closed_g = closed_g_and_mapping.first;
  bidict<KwargDataflowGraphInput<GraphInputName>, Node> closed_mapping = closed_g_and_mapping.second;

  std::function<nlohmann::json(Node const &)> closed_render_node =
    [&](Node const &n) -> nlohmann::json {
      if (closed_mapping.contains_r(n)) {
        return render_value(OpenKwargDataflowValue<GraphInputName, SlotName>{closed_mapping.at_r(n)});
      } else {
        return render_node(n);
      }
    };

  std::function<nlohmann::json(KwargDataflowOutput<std::optional<SlotName>> const &)> closed_render_value =
    [&](KwargDataflowOutput<std::optional<SlotName>> const &o) -> nlohmann::json {
  ...
  std::function<nlohmann::json(std::optional<SlotName> const &)> closed_render_slot_name =
  ...
  std::function<std::vector<std::optional<SlotName>>(std::unordered_set<std::optional<SlotName>> const &)>
    closed_order_slots = [&](std::unordered_set<std::optional<SlotName>> const &unsorted)
      -> std::vector<std::optional<SlotName>>
    {
      std::unordered_set<SlotName> not_nullopt =
        filtrans(unsorted,
                 [](std::optional<SlotName> const &s)
                   -> std::optional<SlotName>
                 {
                   return s;
                 });
      ...
      std::vector<std::optional<SlotName>> result =
        transform(sorted_not_nullopt,
                  [](SlotName const &s) -> std::optional<SlotName> {
                    return s;
                  });
