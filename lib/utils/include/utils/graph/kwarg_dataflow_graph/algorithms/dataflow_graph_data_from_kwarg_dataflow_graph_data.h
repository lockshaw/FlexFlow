...
DataflowGraphData
  dataflow_graph_data_from_kwarg_dataflow_graph_data(
...
{
  std::unordered_set<KwargDataflowInput<SlotName>> all_inputs =
    transform(
      kwarg_data.edges,
  ...
  std::unordered_map<Node, std::unordered_set<SlotName>> incoming_slots_by_node =
      map_values(
        group_by(all_inputs,
  ...
  auto dataflow_input_from_kwarg_input = [&](KwargDataflowInput<SlotName> const &i)
    -> DataflowInput
  {
    std::vector<SlotName> slot_ordering = order_slots(incoming_slots_by_node.at(i.node));
  ...
  auto dataflow_output_from_kwarg_output = [&](KwargDataflowOutput<SlotName> const &o)
    -> DataflowOutput
  {
    std::vector<SlotName> slot_ordering = order_slots(outgoing_slots_by_node.at(o.node));
  ...
  DataflowGraphData result_data = DataflowGraphData{
    ...
    /*edges=*/transform(kwarg_data.edges, dataflow_edge_from_kwarg_dataflow_edge),
    /*outputs=*/transform(kwarg_data.outputs, dataflow_output_from_kwarg_output),
  };
