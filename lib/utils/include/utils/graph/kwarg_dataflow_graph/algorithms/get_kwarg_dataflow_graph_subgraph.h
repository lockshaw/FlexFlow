...
{
  KwargDataflowGraphData<SlotName> g_data =
    get_kwarg_dataflow_graph_data(g);

  std::unordered_set<Node> nodes = set_intersection(g_data.nodes, subgraph_nodes);

  std::unordered_set<KwargDataflowEdge<SlotName>> edges
    = filter(g_data.edges,
             [&](KwargDataflowEdge<SlotName> const &e) -> bool {
               return contains(subgraph_nodes, e.src.node) && contains(subgraph_nodes, e.dst.node);
             });

  std::unordered_set<KwargDataflowOutput<SlotName>> outputs =
    filter(g_data.outputs,
