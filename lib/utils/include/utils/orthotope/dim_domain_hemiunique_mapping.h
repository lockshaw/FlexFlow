...
template <typename L, typename R>
std::ostream &operator<<(std::ostream &s, DimDomainHemiuniqueMapping<L, R> const &m) {
  CHECK_FMTABLE(L);
  CHECK_FMTABLE(R);
  ...
  DimDomainBiuniqueMapping<L, R> const &m)
{
  return DimDomainHemiuniqueMapping<L, R>{
    /*coord_mapping=*/HemiuniqueBinaryRelation<DimCoord<L>, DimCoord<R>>{m.coord_mapping},
    /*l_domain=*/m.l_domain,
    /*r_domain=*/m.r_domain,
  };
}
...
template <typename L, typename R>
DimDomainHemiuniqueMapping<L, R>
  dim_domain_hemiunique_mapping_by_scaling_projection(DimProjection<L, R> const &projection,
                                                      DimDomain<L> const &l_domain,
                                                      DimDomain<R> const &r_domain,
                                                      DimOrdering<L> const &l_dim_ordering,
                                                      DimOrdering<R> const &r_dim_ordering)
{
  ...
  auto have_differing_dim_sizes = 
    [&](L l_dim, R r_dim) -> bool {
  ...
  bidict<L, R> with_matching_dim_sizes = filter_bidict(
    eq_proj.dim_mapping,
    [&](L const &l, R const &r) -> bool {
      return !have_differing_dim_sizes(l, r);
    });

  bidict<L, R> with_differing_dim_sizes = filter_bidict(
    eq_proj.dim_mapping,
    have_differing_dim_sizes);

  ASSERT(with_differing_dim_sizes.size() == 1);

  std::pair<L, R> differing_dim_pair = get_only(
    unstructured_relation_from_bidict(with_differing_dim_sizes));
  ...
  ManyToOne<nonnegative_int, nonnegative_int> differing_l_to_r = [&]() {
    ...
    return many_to_one_from_unstructured_relation(
        transform(
          unordered_set_of(nonnegative_range(differing_l_dim_size)),
          [&](nonnegative_int l_entry) -> std::pair<nonnegative_int, nonnegative_int> {
            return std::pair{
              l_entry,
  ...
  std::unordered_set<L> nondiffering_l_dims = 
    set_minus(input_dims_of_eq_projection(eq_proj), std::unordered_set{differing_l_dim});

  std::unordered_set<R> nondiffering_r_dims = 
    set_minus(output_dims_of_eq_projection(eq_proj), std::unordered_set{differing_r_dim});

  DimDomainBiuniqueMapping<L, R> mapping_without_differing = dim_domain_biunique_mapping_from_projection(
    DimProjection<L, R>{
      EqProjection<L, R>{
  ...
  auto l_coord_to_r_coord = [&](DimCoord<L> const &l_coord) -> DimCoord<R> {
    DimCoord<L> nondiffer_l_coord = restrict_coord_to_dims(l_coord, nondiffering_l_dims);
    nonnegative_int differ_l_coord = l_coord.raw.at(differing_l_dim);

    DimCoord<R> nondiffer_r_coord = mapping_without_differing.at_l(nondiffer_l_coord);
...
template <typename L, typename R>
DimDomainHemiuniqueMapping<L, R>
    dim_domain_hemiunique_mapping_from_projection(DimProjection<L, R> const &projection,
                                                  DimDomain<L> const &l_domain,
                                                  DimDomain<R> const &r_domain,
                                                  DimOrdering<L> const &l_dim_ordering,
                                                  DimOrdering<R> const &r_dim_ordering) {
  if (dim_domain_volume(l_domain) == dim_domain_volume(r_domain)) {
    DimDomainBiuniqueMapping<L, R> biunique = dim_domain_biunique_mapping_from_projection(
      projection,
      l_domain,
      r_domain,
      l_dim_ordering,
    ...
    PANIC(
      "Not sure how to handle this case, but I don't think it should occur in practice. "
...
struct hash<::FlexFlow::DimDomainHemiuniqueMapping<L, R>> {
  size_t operator()(
      ::FlexFlow::DimDomainHemiuniqueMapping<L, R> const &dim_domain_mapping) const {
