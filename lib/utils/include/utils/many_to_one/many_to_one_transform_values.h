...
template <typename L, typename R1, typename F, typename R2 = std::invoke_result_t<F, R1>>
ManyToOne<L, R2> many_to_one_transform_values(
  ManyToOne<L, R1> const &input,
  F &&f)
{
  return many_to_one_from_unstructured_relation(
    transform(
      unstructured_relation_from_many_to_one(input),
