...
MODEL_DIRECTORY = "/home/groups/aaiken/unger/models/nasrnn-2-mini"
...
def top_level_task():
  ...
  dims_input = [ffconfig.get_batch_size(), 512]
  input = ffmodel.create_tensor(dims_input, DataType.DT_FLOAT)
  state = ffmodel.create_tensor(dims_input, DataType.DT_FLOAT)
  ...
  onnx_filename = f"{MODEL_DIRECTORY}/nasrnn{model_tag}_{ffconfig.get_batch_size()}_{optimized_suffix}.onnx"
  print(f'Loading onnx file: {onnx_filename}')
  ...
  ffmodel.compile(loss_type=LossType.LOSS_MEAN_SQUARED_ERROR_SUM_REDUCE, metrics=[MetricsType.METRICS_MEAN_SQUARED_ERROR])
  ...
  #full_input_np = np.zeros((num_samples, 1, 512), dtype=np.float32)
  full_input_np = np.random.rand(num_samples, 512)
  full_label_np = np.random.rand(num_samples, 512)
...
#     full_input_np[i, :, :] = np.random.rand(1, 512)
...
#   full_label_np = y_train
  ...
  dims_full_input = [num_samples, 512]
  full_input = ffmodel.create_tensor(dims_full_input, DataType.DT_FLOAT)

  dims_full_label = [num_samples, 512]
  full_label = ffmodel.create_tensor(dims_full_label, DataType.DT_FLOAT)
  ...
  dataloader_label = SingleDataLoader(ffmodel, label, full_label, num_samples, DataType.DT_FLOAT)
